...
library(lubridate)
```
Hi there! What follows is a written out methodology for *Injustice Watch's* sentencing analysis on Criminal Division judges. All work was done in R and Stata. The R code and Stata code can be downloaded at our GitHub; since our data is read in from the Cook County Online Data portal, our work can easily be re-created or expanded upon. Thanks!
...
original_subset$counter_F1 <-ifelse(original_subset$CLASS==1, 1, 0)
original_subset$counter_F2 <-ifelse(original_subset$CLASS==2, 1, 0)
original_subset$counter_F3 <-ifelse(original_subset$CLASS==3, 1, 0)
original_subset$counter_F4 <-ifelse(original_subset$CLASS==4, 1, 0)
original_subset$counter_F4_pj <-ifelse(original_subset$CLASS==4 & original_subset$SENTENCE_TYPE!="Probation", 1, 0)
judge_rankings<-aggregate(original_subset[45:52], by=list(judges=original_subset$SENTENCE_JUDGE), FUN=sum, na.rm=TRUE)
judge_rankings<-subset(judge_rankings, judge_rankings$counter>=1000)
